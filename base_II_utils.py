...
def trunc_feature(feature=None, stop=None):
    """
    ...
    stop : int
        number of feature letters to print before adding ellipsis (default None)
    ...
    for k in range(rows):
        for p in range(cols):
            if (p + k * cols) < len(top_feat):
                axxx[k, p].set_title(f'{trunc_feature(top_feat[p + k * cols], 10)}', fontsize=fontsize)
...
#############################################
def cramer_heatmap(cramer_corr, cramer_threshold, datatype, categorical, folder_dir):
    """
    ...
    cramer_hm.figure.savefig(folder_dir + f"/{datatype}_cramer_hm.tiff", dpi=300, bbox_inches='tight')
    plt.close(cramer_hm.figure)


def spearman_heatmap(spearman_corr, spearman_thresh, datatype, continuous, folder_dir):
    """
    ...
    spearman_hm.figure.savefig(folder_dir + f"/{datatype}_spearman_hm.tiff", dpi=300, bbox_inches='tight')
    plt.close(spearman_hm.figure)


def pbs_heatmap(pbs_corr, pbs_threshold, datatype, remaining_cats, remaining_cont, longer, folder_dir):
    """
    ...
    if longer == remaining_cats:
        ...
        plt.xlabel('# Categorical feature', fontsize=10)
        plt.ylabel('# Continuous feature', fontsize=10)
    pbserial_hm.figure.savefig(folder_dir + f"/{datatype}_pbserial_hm.tiff", dpi=300, bbox_inches='tight')
